...
*   **Signers**&#x20;
    ...
    * **require\_rg** (boolean): Se true, exibe o campo de RG para o signatário confirmar/preencher na tela de assinatura.
    * **rg** (string): Número do RG do signatário. Diferente do `cpf`, esse campo aceita qualquer valor em qualquer formato — a API não faz validação de formato nem dígito verificador. Nota: o RG só aparece no Relatório de Assinaturas se `require_rg` for enviado como `true`. Enviar o `rg` sem essa flag salva o valor, mas ele não é exibido no relatório final.
